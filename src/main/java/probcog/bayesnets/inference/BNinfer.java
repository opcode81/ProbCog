...
public class BNinfer {
	...
	public static void main(String[] args) throws ProbCogException {
		...
		for(Entry<String,String> entry : db.getEntries()) {
			...
			if(node == null)
				throw new ProbCogException("Evidence node '" + entry.getKey() + "' not found in model.");
			Discrete dom = (Discrete)node.getDomain();
			int domidx = dom.findName(entry.getValue());
			if(domidx == -1)
				throw new ProbCogException("Value '" + entry.getValue() + "' not found in domain of node '" + entry.getKey() + "'");
		...
		for(String qq : queries) {
			...
			if(varIdx == -1)
				throw new ProbCogException("Unknown variable '" + qq + "'");
